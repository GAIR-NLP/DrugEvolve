...
class IEGMN(nn.Module):
    ...
    def __init__(self, n_lays, debug, device, use_rec_atoms, shared_layers, noise_decay_rate, cross_msgs, noise_initial,
        ...
        # A ligand-conditioned pocket anchor used only in the final receptor
        # keypoint readout.  The anchor is a soft barycenter of receptor residue
        # coordinates, so it is translation/rotation equivariant; only invariant
        # squared distances to it are injected back into attention logits.
        self.pocket_attention_rec = nn.Sequential(
            nn.Linear(self.out_feats_dim, self.out_feats_dim, bias=False))
        self.pocket_queries_rec = nn.Sequential(
            nn.Linear(self.out_feats_dim, self.out_feats_dim, bias=False))
        # softplus(logit) controls how strongly receptor keypoints are localized
        # around the learned pocket anchor.  Start mildly local, not collapsed.
        self.keypoint_local_strength = nn.Parameter(torch.tensor(-2.0))
    ...
    def forward(self, lig_graph, rec_graph, geometry_graph, complex_names, epoch):
        ...
        for idx in range(len(ligs_node_idx) - 1):
            ...
            # First predict a single ligand-conditioned receptor pocket anchor.
            # This is not a new supervised head: it is a differentiable, equivariant
            # intermediate used to bias the multi-head keypoint readout toward one
            # physically coherent pocket instead of allowing diffuse global heads.
            pocket_logits = (self.pocket_attention_rec(rec_feats) @
                             self.pocket_queries_rec(lig_feats_mean).t() /
                             math.sqrt(d)).view(-1)
            pocket_weights = torch.softmax(pocket_logits, dim=0)
            pocket_center = pocket_weights.unsqueeze(0) @ Z_rec_coords  # (1, 3)
            rec_local_dist2 = torch.sum((Z_rec_coords - pocket_center) ** 2, dim=1)

            # Att weights to compute receptor keypoints.  The learned radial bias
            # uses only squared distances to the equivariant pocket anchor, so the
            # logits remain invariant and the weighted coordinates remain equivariant.
            att_logits_rec = (self.keypts_attention_rec(rec_feats).view(-1, self.num_att_heads, d).transpose(0, 1) @
                              self.keypts_queries_rec(lig_feats_mean).view(1, self.num_att_heads, d).transpose(0,
                                                                                                               1).transpose(
                                  1, 2) /
                              math.sqrt(d)).view(self.num_att_heads, -1)
            local_strength = torch.nn.functional.softplus(self.keypoint_local_strength)
            att_logits_rec = att_logits_rec - local_strength * rec_local_dist2.unsqueeze(0) / 64.0
            att_weights_rec = torch.softmax(att_logits_rec, dim=1)
            ...
            ligs_feats.append(lig_feats)
            recs_feats.append(rec_feats)
        ...
        unstable_mask = ((S_batch.min(dim=1).values < 1e-3) |
                         (singular_value_differences.amin(dim=(1, 2)) < 1e-2))

        # Common case: nothing unstable -> use the batched SVD directly (no CPU sync loop,
        # no per-item re-stack). Only patch the rare unstable complexes with jitter+re-SVD.
        final_covariances = covariances
        if bool(unstable_mask.any()):
            unstable = unstable_mask.tolist()
            cov_list = list(covariances.unbind(0))
            u_list = list(U_batch.unbind(0))
            vt_list = list(Vt_batch.unbind(0))
            for idx, is_unstable in enumerate(unstable):
                if not is_unstable:
                    continue
                A = cov_list[idx]; S = S_batch[idx]; U = u_list[idx]; Vt = vt_list[idx]
                num_it = 0
                ...
                cov_list[idx] = A; u_list[idx] = U; vt_list[idx] = Vt
            final_covariances = torch.stack(cov_list)
            U_batch = torch.stack(u_list)
            Vt_batch = torch.stack(vt_list)
        ...
        return [rotations, translations, ligs_keypts, recs_keypts, ligs_evolved, geom_losses, ligs_feats, recs_feats]
...
class EquiBind(nn.Module):
    ...
    def __init__(self, device='cuda:0', debug=False, use_evolved_lig=False, evolve_only=False, **kwargs):
        ...
        # Post-Kabsch equivariant contact refinement.  It operates only after the
        # coarse rigid pose has put ligand and receptor into a common frame; then
        # receptor-relative directions are physically meaningful.  The final
        # displacement head is zero-initialized, so the initial function is exactly
        # the original EquiBind pose and training can learn a correction only if it
        # improves the converged objective.
        refine_dim = kwargs.get("iegmn_lay_hid_dim", 64)
        refine_dropout = kwargs.get("dropout", 0.0)
        refine_num_rbf = 16
        self.post_kabsch_refinement = True
        self.refine_topk = 64
        self.refine_max_step = 1.5
        self.refine_distance_cutoff = 16.0
        rbf_width = self.refine_distance_cutoff / float(refine_num_rbf - 1)
        self.refine_rbf_gamma = 1.0 / max(rbf_width * rbf_width, 1.0e-6)
        self.register_buffer(
            "refine_rbf_centers",
            torch.linspace(0.0, self.refine_distance_cutoff, refine_num_rbf),
        )
        self.refine_pair_mlp = nn.Sequential(
            nn.Linear(2 * refine_dim + refine_num_rbf, refine_dim),
        ...
        self.refine_score = nn.Linear(refine_dim, 1)
        self.refine_step = nn.Linear(refine_dim, 1)

        self.iegmn = IEGMN(device=self.device, debug=debug, **kwargs)
        self._zero_refiner_output()
    ...
    def reset_parameters(self):
        for p in self.parameters():
            if p.dim() > 1:
                ...
                torch.nn.init.zeros_(p)
        for module in self.refine_pair_mlp.modules():
            if isinstance(module, nn.LayerNorm):
                nn.init.ones_(module.weight)
                nn.init.zeros_(module.bias)
        self._zero_refiner_output()

    def _zero_refiner_output(self):
        nn.init.zeros_(self.refine_score.weight)
        nn.init.zeros_(self.refine_score.bias)
        nn.init.zeros_(self.refine_step.weight)
        nn.init.zeros_(self.refine_step.bias)

    def _post_kabsch_refine(self, predicted_coords, lig_feats, rec_feats, rec_coords):
        if (not self.post_kabsch_refinement) or predicted_coords.numel() == 0 or rec_coords.numel() == 0:
            return predicted_coords, predicted_coords.new_zeros(())

        n_lig = predicted_coords.shape[0]
        n_rec = rec_coords.shape[0]

        full_diff = rec_coords.unsqueeze(0) - predicted_coords.unsqueeze(1)  # (n, m, 3)
        full_dist2 = torch.sum(full_diff * full_diff, dim=-1)
        k = min(self.refine_topk, n_rec)

        if k < n_rec:
            nbr_idx = torch.topk(full_dist2, k=k, dim=1, largest=False).indices
            rec_coords_local = rec_coords[nbr_idx]          # (n, k, 3)
            rec_feats_local = rec_feats[nbr_idx]            # (n, k, d)
            diff = rec_coords_local - predicted_coords.unsqueeze(1)
            dist2 = torch.sum(diff * diff, dim=-1).clamp(min=1.0e-8)
        else:
            rec_feats_local = rec_feats.unsqueeze(0).expand(n_lig, n_rec, rec_feats.shape[-1])
            diff = full_diff
            dist2 = full_dist2.clamp(min=1.0e-8)

        dist = torch.sqrt(dist2)
        direction = diff / dist.unsqueeze(-1).clamp(min=1.0e-4)

        centers = self.refine_rbf_centers.to(
            device=predicted_coords.device, dtype=predicted_coords.dtype
        ).view(1, 1, -1)
        rbf = torch.exp(-((dist.unsqueeze(-1) - centers) ** 2) * self.refine_rbf_gamma)

        lig_pair_feats = lig_feats.unsqueeze(1).expand(-1, k, -1)
        pair_input = torch.cat((lig_pair_feats, rec_feats_local, rbf), dim=-1)
        hidden = self.refine_pair_mlp(pair_input)

        # Distance bias keeps the learned field local and prevents distant residues
        # from dominating when the coarse pose is already near the pocket.
        logits = self.refine_score(hidden).squeeze(-1) - 0.25 * dist
        weights = torch.softmax(logits, dim=1)

        step = self.refine_max_step * torch.tanh(self.refine_step(hidden).squeeze(-1))
        update = (weights.unsqueeze(-1) * step.unsqueeze(-1) * direction).sum(dim=1)
        update = torch.nan_to_num(update, nan=0.0, posinf=0.0, neginf=0.0)

        update_norm = update.norm(dim=-1, keepdim=True).clamp(min=1.0e-6)
        update = update * torch.clamp(self.refine_max_step / update_norm, max=1.0)

        refined = predicted_coords + update
        # Tiny mechanism-local regularizer: allows flexible correction but makes
        # the zero-initialized baseline solution cheap unless the contact field
        # truly improves coordinate/Kabsch losses.
        residual_penalty = 1.0e-3 * torch.mean(torch.sum(update * update, dim=-1))
        return refined, residual_penalty
    ...
    def forward(self, lig_graph, rec_graph, geometry_graph=None, complex_names=None, epoch=0):
        ...
        lig_feats_by_complex = outputs[6] if len(outputs) > 6 else None
        rec_feats_by_complex = outputs[7] if len(outputs) > 7 else None
        geom_penalty = lig_graph.ndata['new_x'].new_zeros(())
        ...
        for idx in range(len(ligs_node_idx) - 1):
            ...
            if self.post_kabsch_refinement and lig_feats_by_complex is not None and rec_feats_by_complex is not None:
                rec_start = recs_node_idx[idx]
                rec_end = recs_node_idx[idx + 1]
                rec_coords = rec_graph.ndata['x'][rec_start:rec_end]
                predicted_coords, penalty = self._post_kabsch_refine(
                    predicted_coords,
                    lig_feats_by_complex[idx],
                    rec_feats_by_complex[idx],
                    rec_coords,
                )
                geom_penalty = geom_penalty + penalty

            predicted_coords = torch.nan_to_num(
                predicted_coords, nan=0.0, posinf=1.0e4, neginf=-1.0e4
            )
            predicted_ligs_coords_list.append(predicted_coords)

        return predicted_ligs_coords_list, outputs[2], outputs[3], outputs[0], outputs[1], outputs[5] + geom_penalty
